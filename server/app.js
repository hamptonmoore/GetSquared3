...
class player {
	constructor(id, x, y, color, type) {
		...
		this.friction = 0.95;
		this.markers = [];
		this.username = [randomInt(0, 173), randomInt(0, 173)];
		this.points = 0;
		this.type = type;
	}
	...
	initTransportify() {
		let transport = [
			...
			Math.floor(this.x),
			Math.floor(this.y),
			Math.round(this.xm * 100),
			Math.round(this.ym * 100),
			this.color.r,
			this.color.g,
			this.color.b,
			this.points,
	...
	spawnMarker() {
		...
		this.markers.push(
			new marker(this.x + this.width / 2, this.y + this.width / 2, this.id)
		);
	}

	colCheck(shapeA) {
		if (
			shapeA.x < this.x + this.width &&
	...
	updatePhysics(ws) {
		...
		if (boxCol) {
			for (var i in game.clients) {
				...
				var bounds = [
					Math.min(this.markers[0].x, this.markers[1].x),
					Math.min(this.markers[0].y, this.markers[1].y)
				];
				var col = game.clients[i].colCheck({
					x: bounds[0],
					y: bounds[1],
					width: Math.max(this.markers[0].x, this.markers[1].x) - bounds[0],
					height: Math.max(this.markers[0].y, this.markers[1].y) - bounds[1]
				});
		...
		if (this.markers[0]) {
			if (
				Math.floor(new Date().getTime() / 1000) - this.markers[0].spawnTime >
				10
			) {
				this.markers.shift();
			}
...
setInterval(function() {
	...
	for (let i in game.clients) {
		...
		if (game.clients[i].type == "demo") {
			sockets[i].send(
				new Int16Array([102].concat(game.clients[i].quickTransportify()))
			);
		} else {
